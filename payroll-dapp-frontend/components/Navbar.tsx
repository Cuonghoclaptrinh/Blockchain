...
export default function Navbar() {
    ...
    // === THEO ĐÚNG Ý BẠN: CHỈ 2 NÚT MỖI LOẠI ===
    const employeeLinks = [
        { href: '/employee', label: 'Chấm công' },
        { href: '/history', label: 'Lịch sử' },
    ];
    ...
    const links = isOwner ? adminLinks : employeeLinks;

    // Nếu chưa connect ví → chỉ hiện logo + connect
    if (!isConnected) {
        return (
    ...
    return (
        <nav className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
                ...
                {/* 2 NÚT DUY NHẤT – THEO Ý BẠN */}
                <div className="flex items-center gap-8">
                    {links.map((link) => (
                        <Link
                            key={link.href}
                            href={link.href}
                            className={`text-lg font-semibold transition ${isActive(link.href)
