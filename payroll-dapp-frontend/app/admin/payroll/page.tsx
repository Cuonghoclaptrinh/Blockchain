...
export default function PayrollPage() {
    ...
    const { payAll, payBatch, isOwner, isPending, currency } = usePayrollContract();
    ...
    useEffect(() => {
        ...
        // ⚠️ Không đưa toast vào deps để tránh loop
        fetchData();
    }, [userAddress, publicClient, isOwner, page, reloadKey]); // <- bỏ toast khỏi đây
    ...
    // TRẢ LƯƠNG TẤT CẢ NHÂN VIÊN TRONG TRANG (payAll)
    ...
    // TRẢ LƯƠNG 1 GIAO DỊCH (payBatch)
    const paySuperFast = async () => {
        if (employees.length === 0) return;
        const id = toast.loading(
            `Đang trả lương siêu nhanh cho ${employees.length} người...`,
        );
        try {
            await payBatch(employees.map((e) => e.address));
            toast.dismiss(id);
            toast.success('Trả lương siêu nhanh thành công! (Chỉ 1 giao dịch)');
            setReloadKey(k => k + 1);
        } catch (error: any) {
            toast.dismiss(id);
            toast.error(error.message || 'Trả lương thất bại!');
        ...
        <div className="min-h-screen bg-slate-50">
            <div className="mx-auto max-w-5xl px-4 py-10 space-y-8">
                ...
                <section className="rounded-3xl bg-white p-6 text-center shadow-sm space-y-5">
                    ...
                    {/* Nút trả lương theo index (payAll) */}
                    <button
                    ...
                    {/* Nút trả lương 1 giao dịch (payBatch) */}
                    {employees.length > 0 && (
                        <button
                            onClick={paySuperFast}
                            disabled={isPending}
                            className="w-full rounded-full border border-indigo-500 px-6 py-4 text-sm font-semibold text-indigo-600 shadow-sm transition hover:bg-indigo-50 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                            {isPending
                                ? 'Đang trả lương 1 giao dịch...'
                                : `Trả lương 1 giao dịch cho ${employees.length} người (batch)`}
                        </button>
                    )}
