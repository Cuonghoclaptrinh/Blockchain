...
// export const DEPLOY_BLOCK = 9732538n as const;
...
//             const fromBlock = latestBlock > 9n ? latestBlock - 9n : latestBlock;
//             try {
...
export default function EmployeePage() {
    ...
    const [workedToday, setWorkedToday] = useState(0);
    const [name, setName] = useState('Nhân viên');
    const [rate, setRate] = useState(0); // rate theo ETH/giờ
    const [history, setHistory] = useState<HistoryItem[]>([]);
    const [loading, setLoading] = useState(true);
    const [isEmployee, setIsEmployee] = useState(false);
    const [mounted, setMounted] = useState(false);
    ...
    const handleCheckIn = async () => {
        ...
        try {
            ...
            toast.dismiss(id);
            toast.error(e?.shortMessage || e?.message || 'Check-in thất bại!');
        }
    };
    ...
    const handleCheckOut = async () => {
        ...
        try {
            ...
            toast.dismiss(id);
            toast.error(e?.shortMessage || e?.message || 'Check-out thất bại!');
        }
    };
    ...
    const handleWithdraw = async () => {
        ...
        try {
            ...
            toast.dismiss(id);
            toast.error(e?.shortMessage || e?.message || 'Rút lương thất bại!');
        }
    };
    ...
    // UI chính (phần dưới của bạn giữ nguyên)
    return (
        <div className="min-h-screen bg-slate-50">
            <div className="mx-auto max-w-2xl px-4 py-10 space-y-6">
                {/* HEADER / PROFILE */}
                ...
                <section className="rounded-2xl bg-white p-6 shadow-sm">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                        Dữ liệu được đọc từ mảng attendanceHistory on-chain.
                    </p>
